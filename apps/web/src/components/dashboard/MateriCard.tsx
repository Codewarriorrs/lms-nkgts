...
export function MateriCard({ item }: MateriCardProps) {
  const targetHref = item.slug ? `/dashboard/materi/${item.slug}` : "/dashboard/materi";
  return (
    <Link href={targetHref} className="block group">
      <div className="bg-white rounded-xl border border-neutral-100 p-5 hover:border-neutral-200 transition-colors duration-200 cursor-pointer flex flex-col justify-between h-full">
        <div>
